   ...
   let city = (document.getElementById("cityF") as HTMLInputElement).value;
    ...
    if (skillNodeList.length > 0) {
      ...
      skillNodeList.forEach((skillNode) => {
        ...
        skillsF.innerHTML += `<li contenteditable="true">${skill}</li>`;
      });
    } else {
    ...
    if (experiences.length > 0) {
      ...
      experiences.forEach((experience, index) => {
        ...
        experincehtml += `<div id="jt" contenteditable="true"><strong>${jobTitle}</strong></div>`
        experincehtml += `<div id="j1"> <div id="empN" contenteditable="true"> <p> ${employerName} </p></div> <div id="YtoD" contenteditable="true"> <p> ${processMonth(yearFrom)} - ${processMonth(yearTo)}</p> </div> </div>`
        experincehtml += `<p id="jbl" contenteditable="true">  ${jobLocation}</p>`;
       

        for (let i = 0; i < jobDescriptions.length; i++) {
         experincehtml += "<ul>";  
         experincehtml += `<li id="jobdes" contenteditable="true">${jobDescriptions[i].value}</li>`;
         experincehtml += "</ul>";
        }
...
leftsec1.innerHTML=experincehtml
    ...
    if (educations.length > 0) {
      ...
      educations.forEach((education, index) => {
        ...
        educationall += `<div id="eduels"><div id="eduls" contenteditable="true">${educationLevel} in </div> <div id="edus" contenteditable="true">${subject}</div></div>`;
        educationall += `<div id="eduip"><div id="eduin" contenteditable="true">${instituteName}</div> <div id="edupoy" contenteditable="true">(${processMonth(passedOutYear)})</div> </div>`;
        educationall += `<div id="edui" contenteditable="true">${instituteLocation}</div>`
